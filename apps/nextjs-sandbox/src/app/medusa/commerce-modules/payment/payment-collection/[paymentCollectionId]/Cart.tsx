...
export const Cart = (props: CartProps) => {
  ...
  const getCartByPaymentCollectionIdQuery = useQuery({
    queryKey: [CartQK.GET_CART_BY_PAYMENT_COLLECTION_ID, paymentCollectionId],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (getCartByPaymentCollectionIdQuery.data) {
      setCartRegionId(getCartByPaymentCollectionIdQuery.data.cart.region.id);
    }
  ...
  return (
    ...
    >
      ...
      <div>
        Sales Channel: {cart.sales_channel.name} ({cart.sales_channel_id})
      </div>
      {cart.completed_at ? (
