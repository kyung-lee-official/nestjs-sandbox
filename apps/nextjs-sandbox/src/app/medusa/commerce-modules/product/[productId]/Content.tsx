...
import { getProductById, ProductQK } from "../api";
  ...
  return (
    <div>
      ...
      <AddToCart
        ...
        cartId={getCartByRegionIdAndSalesChannelIdAndCustomerIdQuery.data?.id}
        regionId={regionId}
          ...
          <Tbody>
            {productQuery.data?.variants.map((variant: any) => (
              <tr key={variant.id}>
                ...
                <td>
                  {dayjs(variant.created_at).format("YYYY-MM-DD HH:mm:ss")}
                </td>
                <td>
                  {dayjs(variant.updated_at).format("YYYY-MM-DD HH:mm:ss")}
                </td>
              </tr>
            ))}
          </Tbody>
